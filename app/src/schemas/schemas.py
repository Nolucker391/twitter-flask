...
tweet_model = api.model("Tweet", {
    ...
    "author": fields.Nested(author_model, required=True, description="Author information"),
    "likes": fields.List(fields.Nested(like_model), required=False, description="List of likes")
})

# 4. Модель для "TweetGetResponse" запроса для GET /api/tweets/
tweet_get_response_model = api.model("TweetGetResponse", {
    "result": fields.Boolean(required=True, description="Result status"),
    "tweets": fields.List(fields.Nested(tweet_model), required=True, description="List of tweets with attachments")
})
...
following_model = api.model("Following", {
    "id": fields.Integer(required=True, description="ID of the user being followed"),
    "name": fields.String(required=True, description="Name of the user being followed")
})

# Модель "UserDetails" для "User"
user_detail = api.model("UserDetails", {
    "id": fields.Integer(required=True, description="ID of the user"),
    "name": fields.String(required=True, description="Name of the user"),
    "followers": fields.List(fields.Nested(follower_model), required=True, description="List of followers"),
    "following": fields.List(fields.Nested(following_model), required=True, description="List of users being followed"),
...
user_information = api.model("User", {
    "result": fields.Boolean(required=True, description="Result status"),
    "user": fields.Nested(user_detail, required=True, description="Detailed user information")
})
