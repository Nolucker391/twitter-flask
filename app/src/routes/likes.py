...
class LikeTweetResource(Resource):
    def process_like(self, tweet_id, query_method):
        ...
        try:
            ...
            if not api_key:
                logger.error(
                    f"DELETE Запрос на /tweets/{tweet_id}/like не обработан. Пользователь с ключом <{api_key}> не найден."
                )
                return {
                    "result": False,
            ...
            result, status_code = db_queries.add_delete_like(
                api_key=api_key, tweet_id=tweet_id, query_method=query_method
            )
