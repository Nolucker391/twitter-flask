...
from app.src.database.models import User, ApiKey, Tweets, Like
...
class QueriesDatabase:
    ...
    def add_delete_like(self, api_key, tweet_id, query_method):
        ...
        try:
            if query_method == "add":
                self.session.execute(insert(Like).values(user_id=user.id, tweet_id=tweet_id))
            elif query_method == "delete":
                self.session.execute(delete(Like).filter(Like.tweet_id == tweet_id, Like.user_id == user.id))
            else:
